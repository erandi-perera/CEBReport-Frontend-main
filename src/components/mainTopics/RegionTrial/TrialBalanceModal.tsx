                    ...
                    return (
                      <React.Fragment key={category}>
                        ...
                        <tr className="bg-gradient-to-r from-gray-200 to-gray-300 font-bold border-t-2 border-[#7A0000]">
                          <td className="px-2 py-1 text-xs print:hidden font-bold" colSpan={1}>TOTAL {getCategoryDescription(category)}</td>
                          <td className="px-2 py-1 text-xs font-bold" colSpan={1}></td>
                          <td className="px-2 py-1 text-right font-mono font-bold">{formatNumber(categoryTotals[category.charAt(0).toUpperCase()]?.opening || 0)}</td>
                          <td className="px-2 py-1 text-right font-mono font-bold">{formatNumber(categoryTotals[category.charAt(0).toUpperCase()]?.debit || 0)}</td>
                          <td className="px-2 py-1 text-right font-mono font-bold">{formatNumber(categoryTotals[category.charAt(0).toUpperCase()]?.credit || 0)}</td>
                          <td className="px-2 py-1 text-right font-mono font-bold">{formatNumber(categoryTotals[category.charAt(0).toUpperCase()]?.closing || 0)}</td>
                        </tr>
                      </React.Fragment>
                    );
                  })}
