...
const PHVObsoleteIdleBOS: React.FC = () => {
  const { user } = useUser();
  ...
  // Report state
  const [reportData, setReportData] = useState<ObsoleteIdleBOSItem[]>([]);
  ...
  const handleViewReport = async () => {
    ...
    try {
      ...
      const items: ObsoleteIdleBOSItem[] = Array.isArray(json) ? json : json.data || [];
      setReportData(items);
      items.length === 0
  ...
  const handleDownloadCSV = () => {
    ...
    rows.push(`"Board of Survey Recommendation for Obsolete AND Idle Stocks - Annual Verification of the Year-${selectedYear}"`);
    ...
    link.download = `PHV_ObsoleteIdle_BOS_${selectedDept.DeptId}_${selectedWarehouse}_${selectedYear}.csv`;
    link.click();
  };
  ...
  const printPDF = () => {
    ...
    // ── PAGINATION ──────────────────────────────────────────────────────────
    const chunks: ObsoleteIdleBOSItem[][] = [];
    let current: ObsoleteIdleBOSItem[] = [];
    let usedH = 0;
    ...
    const headerHTML = `
      <div class="header">
        ...
        <div class="header-title">
          Board of Survey Recommendation for Obsolete AND Idle Stocks - Annual Verification of the Year-${selectedYear}
        </div>
      </div>`;
  ...
  return (
    <div className="max-w-[95%] mx-auto p-2 md:p-4 bg-white rounded-xl shadow border border-gray-200 text-sm md:text-base font-sans">
      ...
      {showReport && selectedDept && (
        <ReportViewer
          title={`PHV OBSOLETE / IDLE BOS - ${selectedYear}`}
...
export default PHVObsoleteIdleBOS;