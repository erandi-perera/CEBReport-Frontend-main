...
`\t${item.MaterialCode || ""}`,  
  ...
  const printPDF = () => {
    ...
    const FULL_HEADER_HEIGHT = 22;    
    const REPEAT_HEADER_HEIGHT = 16;  
    ...
    const SUBSEQUENT_PAGE_TABLE_HEIGHT =
      PAGE_HEIGHT -
      MARGIN_TOP -
      MARGIN_BOTTOM -
      REPEAT_HEADER_HEIGHT -
      TABLE_HEADER_HEIGHT -
      SIGNATURE_HEIGHT -
      FOOTER_HEIGHT -
      SECTION_SPACING * 2;

    const ROWS_PER_FIRST_PAGE = Math.floor(FIRST_PAGE_TABLE_HEIGHT / ROW_HEIGHT);
    const ROWS_PER_SUBSEQUENT_PAGE = Math.floor(SUBSEQUENT_PAGE_TABLE_HEIGHT / ROW_HEIGHT);
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page {
      position: relative;
      min-height: ${PAGE_HEIGHT - MARGIN_TOP - MARGIN_BOTTOM}mm;
      display: flex;
  ...
  `;
    ...
    <thead>
      <tr>
        ...
        <th class="stock-val">Stock Book Val</th>
        <th class="phys-val">Physical Val</th>
        <th class="surplus-val">Surplus Val</th>
        <th class="short-val">Shortage Val</th>
      </tr>
    </thead>
  `;
  ...
  `;
    ...
    <div class="footer-section">
      <div class="footer">
        <div>Date & time of the Report Generated: ${now}</div>
  ...
  `);
    ...
    if (remainingRows.length > 0) {
      const firstPageRows = remainingRows.splice(0, ROWS_PER_FIRST_PAGE);
      pages.push(firstPageRows);
    }

    // Subsequent pages (with repeated header)
    while (remainingRows.length > 0) {
      const pageRows = remainingRows.splice(0, ROWS_PER_SUBSEQUENT_PAGE);
      pages.push(pageRows);
    }
    ...
    if (pages.length === 0) {
      ...
      const maxRowsOnLastPage = pages.length === 1 ? ROWS_PER_FIRST_PAGE : ROWS_PER_SUBSEQUENT_PAGE;
      if (lastPage.length < maxRowsOnLastPage) {
    ...
    pages.forEach((pageRows, idx) => {
      ...
      html += `
      <div class="page">
        ${isFirstPage ? firstPageHeaderHTML : repeatHeaderHTML}
