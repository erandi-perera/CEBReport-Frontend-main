...
const PHVDamageBOS: React.FC = () => {
  ...
  const handleViewReport = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:44381/api/phv-damage-obs/list?deptId=${encodeURIComponent(
          selectedDept.DeptId
        )}&warehouseCode=${encodeURIComponent(
          selectedWarehouse
        )}&repYear=${selectedYear}&repMonth=${selectedMonth}`
      );
      if (!res.ok) throw new Error(`HTTP ${res.status}: ${await res.text()}`);
      const json = await res.json();
      const items: DamageBOSItem[] = Array.isArray(json) ? json : json.data || [];
      setReportData(items);
      items.length === 0
  ...
  const handleDownloadCSV = () => {
    ...
    reportData.forEach((item, idx) => {
      const row = [
        idx + 1, `\t${item.MaterialCode || ""}`, item.MaterialName || "",
        item.GradeCode || "", item.UomCode || "",
        formatNumber(item.QtyOnHand), formatNumber(item.UnitPrice),
        formatNumber(item.StockBook), "", "", item.Reason || "",
      ];
    ...
    link.download = `PHV_Damage_BOS_${selectedDept.DeptId}_${selectedWarehouse}_${selectedYear}.csv`;
    link.click();
  };
  ...
  const printPDF = () => {
    ...
    // ── Column widths in mm (273mm usable = 297 - 2×12) ──────────────────────
    // These MUST match the CSS .c-* width percentages below
    const USABLE_W = PAGE_W - 2 * M_LR; // 273mm

    const COL_WIDTHS = {
      code: 0.08 * USABLE_W,  // 8%  ≈ 21.8mm
      desc: 0.27 * USABLE_W,  // 27% ≈ 73.7mm
      grade: 0.06 * USABLE_W,  // 6%  ≈ 16.4mm
      uom: 0.05 * USABLE_W,  // 5%  ≈ 13.7mm
      qty: 0.08 * USABLE_W,  // 8%  ≈ 21.8mm
      price: 0.10 * USABLE_W,  // 10% ≈ 27.3mm
    };

    // Conservative char width at 7.5pt Arial ≈ 1.55mm per character
    const CHAR_W_MM = 1.55;
    const LINE_H_MM = 3.6;   // height per line in mm
    const PAD_MM = 2.0;   // top + bottom cell padding total

    /**
     * Calculates row height by checking how many lines each cell needs to wrap,
     * then takes the maximum across all cells — ensuring no content is clipped.
     */
    const getRowHeight = (item: DamageBOSItem): number => {
      const cells: { text: string; colW: number }[] = [
        { text: item.MaterialCode || "", colW: COL_WIDTHS.code },
        { text: item.MaterialName || "", colW: COL_WIDTHS.desc },
        { text: item.GradeCode || "", colW: COL_WIDTHS.grade },
        { text: item.UomCode || "", colW: COL_WIDTHS.uom },
        { text: formatNumber(item.QtyOnHand), colW: COL_WIDTHS.qty },
        { text: formatNumber(item.UnitPrice), colW: COL_WIDTHS.price },
      ];

      const maxLines = cells.reduce((max, { text, colW }) => {
        const effectiveW = colW - 2.4;          // subtract ~1.2mm padding per side
        const charsPerLine = Math.max(1, Math.floor(effectiveW / CHAR_W_MM));
        const lines = Math.max(1, Math.ceil(text.length / charsPerLine));
        return Math.max(max, lines);
      }, 1);

      return PAD_MM + maxLines * LINE_H_MM;
    };

    // ── PAGINATION ────────────────────────────────────────────────────────────
    const chunks: DamageBOSItem[][] = [];
    let current: DamageBOSItem[] = [];
    let usedH = 0;
    ...
    // If total row won't fit on last data page, push an empty overflow page
    const totalRowH = PAD_MM + LINE_H_MM;
    const lastUsed = (chunks[chunks.length - 1] ?? []).reduce(
      (s, it) => s + getRowHeight(it), 0
    );
    if (lastUsed + totalRowH > DATA_AREA_H) chunks.push([]);

    chunks.push([]); // BOS page (always last)
    ...
    const css = `
      ...
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .header-form {
        ...
        border: 0.6pt solid #7A0000; padding: 1mm 2mm; color: #7A0000;
      }
      .header-cc {
        font-size: 8.5pt; font-weight: bold;
        text-align: center; margin-top: 0.8mm; color: #333;
      }
      .header-title {
        font-size: 9pt; font-weight: bold;
        text-align: center; margin-top: 0.5mm; color: #7A0000;
      }

      /* ── TABLE ── */
      .table-area { position: absolute; top: ${HEADER_H}mm; left: 0; right: 0; }
      ...
      th, td {
        ...
        padding: 1mm 1.2mm;
        vertical-align: top;          /* top-align: long cells don't push shorter ones */
      ...
      /* Column widths — percentages must match COL_WIDTHS fractions above */
      .c-sn    { width: 4%;  text-align: center; }
      .c-code  { width: 8%;  text-align: center; font-size: 7pt; }
      .c-desc  { width: 27%; text-align: left;   line-height: 1.3; font-size: 7pt; }
      ...
      /* ── SIGNATURE BLOCK (non-last pages — pinned to bottom) ── */
      .sig-block {
        position: absolute;
        bottom: ${FTR_H + 1}mm;
        left: 0; right: 0;
  ...
  color: #000;
      ...
      .sig-row-bot { display: flex; justify-content: space-between; align-items: flex-start; }
      ...
      .bos-title {
        ...
        text-align: center;
        border-top: 0.8pt solid #000;
        padding-top: 2mm; margin-bottom: 2mm; color: #7A0000;
      }
      .bos-body { text-align: justify; margin-bottom: 3mm; line-height: 1.4; color: #000; }
      .bos-signed-label { font-weight: bold; margin-bottom: 1.5mm; color: #000; }
      .bos-sig-row { display: flex; align-items: flex-start; margin-bottom: 2mm; color: #000; }
      ...
      .bos-approval-row {
        ...
        padding: 5mm 0; border-top: 0.6pt solid #000; color: #000;
      }
...
const sigInnerHTML = `
  <div class="sig-row-top">
    ...
    <div class="sig-col" style="align-items:flex-start;">
      <span>.......................................</span>
  ...
  <div class="sig-row-bot">
    ...
    const sigBlockHTML = `<div class="sig-block">${sigInnerHTML}</div>`;
    const inlineSigHTML = `<div class="bos-sig-block">${sigInnerHTML}</div>`;
    ...
    const bosSectionHTML = `
      <div class="bos-section">
        ...
        <div class="bos-sig-row">
          <div class="bos-sig-name"><div>(1)..................................</div><div>Chairman - BOS(Name)</div></div>
          <div class="bos-sig-right"><div>..................................</div><div>Signature</div></div>
        </div>
        <div class="bos-sig-row">
          <div class="bos-sig-name"><div>(2)..................................</div><div>Member - BOS(Name)</div></div>
          <div class="bos-sig-right"><div>..................................</div><div>Signature</div></div>
        </div>
        <div class="bos-sig-row">
          <div class="bos-sig-name"><div>(3)..................................</div><div>Member - BOS(Name)</div></div>
          <div class="bos-sig-right"><div>..................................</div><div>Signature</div></div>
        </div>
        <div class="bos-sig-row">
          <div class="bos-sig-name"><div>(4)..................................</div><div>Observer - BOS(Name)</div></div>
          <div class="bos-sig-right"><div>..................................</div><div>Signature</div></div>
        </div>
        <div class="bos-date-line">Date &nbsp;: ..................................</div>
    ...
    chunks.forEach((chunk, idx) => {
      ...
      const rowsHTML = chunk.map((item, i) => `
        <tr style="height:${getRowHeight(item)}mm;">
    ...
    <div className="max-w-[95%] mx-auto p-2 md:p-4 bg-white rounded-xl shadow border border-gray-200 text-sm md:text-base font-sans">
      ...
      <div className="flex flex-col md:flex-row flex-wrap gap-2 md:gap-3 mb-4 justify-end items-end">
        ...
        <div className="flex flex-col md:flex-row gap-2 items-end">
          <div className="flex flex-col">
            <label className={`text-xs md:text-sm font-bold ${maroon} mb-1`}>Warehouse Code</label>
            <select
            ...
            >
              ...
              {warehouses.map((wh) => (
                <option key={wh.WarehouseCode} value={wh.WarehouseCode}>{wh.WarehouseCode}</option>
              ))}
            </select>
          </div>
          <button
      ...
      <div className="flex flex-col md:flex-row flex-wrap gap-2 md:gap-3 mb-2 md:mb-4">
        <div className="relative w-full md:w-auto">
          ...
          <input
            type="text" value={searchId} placeholder="Search by Cost Center ID"
        ...
        <div className="relative w-full md:w-auto">
          <Search className="absolute left-2 md:left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-3 h-3" />
          <input
            type="text" value={searchName} placeholder="Search by Name"
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="text-gray-600 bg-gray-100 p-2 md:p-4 rounded text-xs md:text-sm">No cost centers found.</div>
      )}
      ...
      {showReport && selectedDept && (
        <ReportViewer
          title={`PHV DAMAGE BOS - ${selectedYear}`}
