...
const SolarPVBilling: React.FC = () => {
    ...
    const [summaryData, setSummaryData] = useState<SummaryReportData>({ ordinary: [], bulk: [] });
    ...
    useEffect(() => {
        ...
        const fetchCycles = async () => {
            ...
            try {
                ...
                setCycleError(err.message || "Failed to load cycles. Please try again later.");
            } finally {
    ...
    const downloadAsCSV = () => {
        if ((reportType.includes("Detailed") && !detailedData.length) ||
            (reportType === "Summary Report" && (!summaryData.ordinary || !summaryData.bulk))) return;

        // Helper function to safely get property values regardless of naming convention
        const getProperty = (item: any, ...possibleNames: string[]) => {
        ...
        const selectionInfo = selectedCategory === "Entire CEB"
            ? "Entire CEB"
        ...
        if (reportType.includes("Detailed")) {
            ...
            if (reportType === "Detailed Report - Ordinary") {
                headers = [
                    "Division", "Province", "Area", "Name of Customer", "A/C No.",
                    "Type of Customer", "Panel Capacity (kW)", "B/F Units",
                    "Energy Exported (kWh)", "Energy Imported (kWh)", "C/F Units",
                    "Sin No.", "Tariff", "Agreement Date", "Units for Loss Reduction"
                ];
            } else {
                headers = [
                    "Division", "Province", "Area", "Name of Customer", "A/C No.",
            ...
            rows = detailedData.map(item => {
                ...
                row.push(
                    getProperty(item, "CustomerName", "NameOfCustomer"),
                    item.AccountNumber,
                    getProperty(item, "CustomerType", "TypeOfCustomer"),
                    item.PanelCapacity,
                    item.BFUnits,
                    item.EnergyExported,
                    item.EnergyImported,
                    item.CFUnits,
                    getProperty(item, "SinNumber", "SinNo"),
                    item.Tariff,
                    item.AgreementDate
                );
            ...
            const ordinaryByType = summaryData.ordinary.reduce((acc, item) => {
                const type = item.TypeOfCustomer || 'Unknown';
                if (!acc[type]) {
                    acc[type] = {
                        count: 0,
                        capacity: 0,
            ...
            const ordinaryCapacity = summaryData.ordinary.reduce((sum, item) => sum + (item.PanelCapacity || 0), 0);
            const bulkCapacity = summaryData.bulk.reduce((sum, item) => sum + (item.PanelCapacity || 0), 0);
            const totalCapacity = ordinaryCapacity + bulkCapacity;

            const ordinaryExported = summaryData.ordinary.reduce((sum, item) => sum + (item.EnergyExported || 0), 0);
            const bulkExported = summaryData.bulk.reduce((sum, item) => sum + (item.EnergyExported || 0), 0);
            const totalExported = ordinaryExported + bulkExported;

            const ordinaryImported = summaryData.ordinary.reduce((sum, item) => sum + (item.EnergyImported || 0), 0);
            const bulkImported = summaryData.bulk.reduce((sum, item) => sum + (item.EnergyImported || 0), 0);
            ...
            Object.entries(ordinaryByType).forEach(([type, data]) => {
                csvLines.push(`"${data.count}","${type}","${formatCurrency(data.capacity)}","${formatCurrency(data.exported)}","${formatCurrency(data.imported)}"`);
            });

            // Add ordinary total row
            csvLines.push(`"${ordinaryAccounts}","","${formatCurrency(ordinaryCapacity)}","${formatCurrency(ordinaryExported)}","${formatCurrency(ordinaryImported)}"`);

            csvLines.push(""); // Empty line between tables

            // Bulk Customer Details Table
            csvLines.push("Bulk Customer Details");
            csvLines.push(`"No. of Accounts","Type of Customer","Panel Capacity (kW)","Energy Exported (kWh)","Energy Imported (kWh)"`);
            ...
            // Add bulk total row
            csvLines.push(`"${bulkAccounts}","","${formatCurrency(bulkCapacity)}","${formatCurrency(bulkExported)}","${formatCurrency(bulkImported)}"`);

            csvLines.push(""); // Empty line between tables

            // Total Customers Table
            csvLines.push("Total Customers");
            csvLines.push(`"Total Customers","Total Panel Capacity (kW)","Energy Exported (kWh)","Energy Imported (kWh)"`);
            csvLines.push(`"${totalAccounts}","${formatCurrency(totalCapacity)}","${formatCurrency(totalExported)}","${formatCurrency(totalImported)}"`);
        ...
        try {
            ...
            const fileDescriptor = selectedCategory === "Entire CEB"
                ? "EntireCEB"
                : selectedCategory === "Area"
            ...
            link.setAttribute(
                "download",
                `SolarPVConnections_${fileDescriptor}_${selectedCycleType.replace(' ', '')}_${cycleValue}.csv`
            );
    ...
    const renderDetailedTable = () => {
        ...
        const groupedData = detailedData.reduce((acc, item, index) => {
            ...
            if (!acc.divisionCounts[divisionKey]) acc.divisionCounts[divisionKey] = 0;
            ...
            <table className="w-full border-collapse text-xs">
                <thead className="bg-gray-100 sticky top-0">
                    <tr>
                        ...
                        <th className="border border-gray-300 px-2 py-1 text-center">Type of Customer</th>
                        <th className="border border-gray-300 px-2 py-1 text-center">Panel Capacity (kW)</th>
                        <th className="border border-gray-300 px-2 py-1 text-center">B/F Units</th>
                        ...
                        <th className="border border-gray-300 px-2 py-1 text-center">C/F Units</th>
                        <th className="border border-gray-300 px-2 py-1 text-center">Sin No.</th>
                        ...
                        {isOrdinaryReport && (
                            <th className="border border-gray-300 px-2 py-1 text-right">Units for Loss Reduction</th>
                        )}
                    </tr>
                </thead>
                <tbody>
                    {groupedData.items.map((item, index) => {
                        ...
                        return (
                            <tr key={index} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                                ...
                                <td className="border border-gray-300 px-2 py-1">{item.SinNumber}</td>
                                ...
                                {isOrdinaryReport && (
                                    <td className="border border-gray-300 px-2 py-1 text-right">{item.UnitsForLossReduction || ""}</td>
                                )}
                            </tr>
                        );
                    })}
    ...
    const renderSummaryTables = () => {
        ...
        return (
            <div className="space-y-6">
                {/* Ordinary Customer Details Table */}
                <div>
                    <h3 className="font-bold text-lg mb-2 text-[#7A0000]">Ordinary Customer Details</h3>
                    <table className="w-full border-collapse text-xs border border-gray-300">
                        <thead className="bg-gray-100">
                            <tr>
                                <th className="border border-gray-300 px-2 py-1 text-center">No. of Accounts</th>
                                <th className="border border-gray-300 px-2 py-1 text-center">Type of Customer</th>
                                <th className="border border-gray-300 px-2 py-1 text-center">Panel Capacity (kW)</th>
                                <th className="border border-gray-300 px-2 py-1 text-center">Energy Exported (kWh)</th>
                                <th className="border border-gray-300 px-2 py-1 text-center">Energy Imported (kWh)</th>
                        ...
                        <tbody>
                            {Object.entries(ordinaryByType).map(([type, data], index) => (
                                <tr key={index} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                                    <td className="border border-gray-300 px-2 py-1 text-right">{data.count}</td>
                                    <td className="border border-gray-300 px-2 py-1">{type}</td>
                                    <td className="border border-gray-300 px-2 py-1 text-right">{formatCurrency(data.capacity)}</td>
                                    <td className="border border-gray-300 px-2 py-1 text-right">{formatCurrency(data.exported)}</td>
                                    <td className="border border-gray-300 px-2 py-1 text-right">{formatCurrency(data.imported)}</td>
                                </tr>
                            ))}
                            <tr className="font-bold bg-gray-100">
                                <td className="border border-gray-300 px-2 py-1 text-right">{ordinaryAccounts}</td>
                                <td className="border border-gray-300 px-2 py-1"></td>
                                <td className="border border-gray-300 px-2 py-1 text-right">{formatCurrency(ordinaryCapacity)}</td>
                                <td className="border border-gray-300 px-2 py-1 text-right">{formatCurrency(ordinaryExported)}</td>
                                <td className="border border-gray-300 px-2 py-1 text-right">{formatCurrency(ordinaryImported)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                {/* Bulk Customer Details Table */}
                <div>
                    <h3 className="font-bold text-lg mb-2 text-[#7A0000]">Bulk Customer Details</h3>
                    <table className="w-full border-collapse text-xs border border-gray-300">
                        <thead className="bg-gray-100">
                            <tr>
                                <th className="border border-gray-300 px-2 py-1 text-center">No. of Accounts</th>
                        ...
                        <tbody>
                            ...
                            <tr className="font-bold bg-gray-100">
                                <td className="border border-gray-300 px-2 py-1 text-right">{bulkAccounts}</td>
                                <td className="border border-gray-300 px-2 py-1"></td>
                                <td className="border border-gray-300 px-2 py-1 text-right">{formatCurrency(bulkCapacity)}</td>
                                <td className="border border-gray-300 px-2 py-1 text-right">{formatCurrency(bulkExported)}</td>
                                <td className="border border-gray-300 px-2 py-1 text-right">{formatCurrency(bulkImported)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div>
                    <h3 className="font-bold text-lg mb-2 text-[#7A0000]">Total Customers</h3>
                    <table className="w-full border-collapse text-xs border border-gray-300">
                        <thead className="bg-gray-100">
                            <tr>
                                <th className="border border-gray-300 px-2 py-1 text-center">Total Customers</th>
                                <th className="border border-gray-300 px-2 py-1 text-center">Total Panel Capacity (kW)</th>
                                <th className="border border-gray-300 px-2 py-1 text-center">Energy Exported (kWh)</th>
                                <th className="border border-gray-300 px-2 py-1 text-center">Energy Imported (kWh)</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr className="font-bold bg-gray-100">
                                <td className="border border-gray-300 px-2 py-1 text-center">{totalAccounts}</td>
                                <td className="border border-gray-300 px-2 py-1 text-center">{formatCurrency(totalCapacity)}</td>
                                <td className="border border-gray-300 px-2 py-1 text-center">{formatCurrency(totalExported)}</td>
                                <td className="border border-gray-300 px-2 py-1 text-center">{formatCurrency(totalImported)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (reportType === "Detailed Report - Ordinary") {
                ...
                console.log("Making API calls for summary:", ordinaryEndpoint, bulkEndpoint);
                ...
                if (!ordinaryResponse.ok || !bulkResponse.ok) {
                    throw new Error(`Failed to fetch data: Ordinary ${ordinaryResponse.status}, Bulk ${bulkResponse.status}`);
                }

                // Content type verification
                const ordinaryContentType = ordinaryResponse.headers.get("content-type");
                const bulkContentType = bulkResponse.headers.get("content-type");

                if (!ordinaryContentType || !ordinaryContentType.includes("application/json") ||
                    !bulkContentType || !bulkContentType.includes("application/json")) {
                    throw new Error(`Expected JSON but got Ordinary: ${ordinaryContentType}, Bulk: ${bulkContentType}`);
                }
                ...
                const combinedData = {
                    ordinary: Array.isArray(ordinaryResponseData) ? ordinaryResponseData : [ordinaryResponseData],
                    bulk: Array.isArray(bulkResponseData) ? bulkResponseData : [bulkResponseData]
                };
    ...
    const isCycleTypeDisabled = () => {
        ...
        return !categoryValue ||
            ...
            (selectedCategory === "Region" && (isLoadingDivisions || divisionError !== null));
    };

    // Check if month dropdown should be disabled
    const isMonthDisabled = () => {
        return !selectedCycleType || isCycleTypeDisabled() ||
            isLoadingCycles || cycleError !== null;
    };
                ...
                <>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                                    ...
                                    {isLoadingAreas ? (
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setSelectedCategoryName(selectedArea ? selectedArea.AreaName : "");
                                            }}
                                            className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded-md focus:ring-2 focus:ring-[#7A0000] focus:border-transparent"
                                            required
                            ...
                            <div className="flex flex-col">
                                <label
                                    className={`text-xs font-medium mb-1 ${isCycleTypeDisabled()
                                        ? "text-gray-400"
                                        : maroon
                                        }`}
                                >
                            ...
                            <div className="flex flex-col">
                                <label
                                    className={`text-xs font-medium mb-1 ${isMonthDisabled()
                                        ? "text-gray-400"
                                        : maroon
                                        }`}
                                >
                                ...
                                {isLoadingCycles ? (
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => {
                                            ...
                                            const selectedOption = cycleOptions.find(option => option.code === selectedCode);
                                            setSelectedCycleDisplay(selectedOption ? selectedOption.display : "");
                                        }}
                                        className={`w-full px-2 py-1.5 text-xs border rounded-md focus:ring-2 focus:ring-[#7A0000] focus:border-transparent
                                            ${isMonthDisabled()
                                                ? "bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed"
                            ...
                            <div className="flex flex-col">
                                ...
                                >
                                    ...
                                    <option value="Detailed Report - Ordinary">Detailed Report - Ordinary</option>
                                    <option value="Detailed Report - Bulk">Detailed Report - Bulk</option>
