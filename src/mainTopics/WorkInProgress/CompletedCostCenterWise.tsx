...
import { Search, RotateCcw, Eye, ChevronLeft, Download, Printer, Calendar, X } from "lucide-react";
...
const CompletedCostCenterWise = () => {
  ...
  const formatDate = (d?: Date | null) =>
    d ? `${(d.getMonth() + 1).toString().padStart(2, "0")}/${d.getDate().toString().padStart(2, "0")}/${d.getFullYear()}` : "";
  ...
  const parseDateInput = (raw: string): Date | null => {
    ...
    if (iso.test(s)) {
      ...
      if (date && date.getFullYear() === Number(y) && date.getMonth() === Number(m) - 1 && date.getDate() === Number(d)) return date;
      return null;
    }
    ...
    if (slash.test(s)) {
      ...
      if (try1 && try1.getFullYear() === Number(c) && try1.getMonth() === Number(a) - 1 && try1.getDate() === Number(b)) return try1;

      const try2 = new Date(Number(c), Number(b) - 1, Number(a));
      if (try2 && try2.getFullYear() === Number(c) && try2.getMonth() === Number(b) - 1 && try2.getDate() === Number(a)) return try2;
  ...
  const isFuture = (d: Date | null) => (d ? d.getTime() > new Date().setHours(0, 0, 0, 0) : false);
  const daysAgo = (d: Date | null) => {
    if (!d) return 0;
    const today = new Date();
    const diff = Math.floor((today.setHours(0,0,0,0) - new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime()) / (1000 * 60 * 60 * 24));
  ...
  const handleViewCompletedProjects = async (department?: Department) => {
    ...
    if (!targetDepartment || !startDate || !endDate || startError || endError) return;
  ...
  const downloadAsCSV = () => {
    if (!transformedData.length || !selectedDepartment || !startDate || !endDate) return;
    ...
    const csvRows = [
      ...
      ...transformedData.map((item) => [
        ...
        item.DeptId || "",
        item.PIVNo || "",
        item.CloseDate ? new Date(item.CloseDate).toLocaleDateString('en-US') : "",
    ...
    link.href = url;
    link.download = `WIP_CloseJobReport_${selectedDepartment.DeptId}_${startDate.toISOString().slice(0,10)}_${endDate.toISOString().slice(0,10)}.csv`;
    ...
    return (
      <div className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-xl z-50 p-2 min-w-[240px] max-w-[280px]">
        <div className="flex justify-between items-center mb-2 pb-1 border-b border-gray-100">
          <button onClick={prev} className="p-0.5 hover:bg-gray-100 rounded"><ChevronLeft className="w-3 h-3" /></button>
          <h3 className="font-bold text-xs text-gray-800 px-1">{monthNames[m]} {y}</h3>
          <button onClick={next} className="p-0.5 hover:bg-gray-100 rounded"><ChevronLeft className="w-3 h-3 rotate-180" /></button>
        </div>
        <div className="grid grid-cols-7 gap-0.5 text-xs">
          {['Su','Mo','Tu','We','Th','Fr','Sa'].map(d => (
            <div key={d} className="p-0.5 text-center font-semibold text-gray-600 bg-gray-50 rounded">{d}</div>
          ))}
          {days.map((date, i) => {
            ...
            return (
              <button key={i} disabled={disabled} onClick={() => pick(date)}
                className={`p-0.5 text-center rounded text-xs font-medium transition-colors ${
                  isSel ? 'bg-[#7A0000] text-white shadow-sm' :
                  isToday ? 'bg-blue-100 text-blue-600 font-bold border border-blue-300' :
                  disabled ? 'text-gray-300 cursor-not-allowed bg-gray-50' : 'hover:bg-gray-100'
                }`}>
                {date.getDate()}
              </button>
            );
        ...
        <div className="mt-2 pt-1 border-t border-gray-200">
          <button onClick={() => setShowCalendar(null)} className="w-full py-0.5 text-xs text-gray-600 hover:text-gray-800 hover:bg-gray-50 rounded">Close</button>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="w-full p-6 bg-white rounded-xl shadow border border-gray-200 text-sm font-sans relative">
      <div className="flex justify-between items-center mb-4">
        <h2 className={`text-xl font-bold ${maroon}`}>Completed Cost Center Wise</h2>
      </div>
          ...
          <div className="relative">
            ...
            <div className="relative">
              <input
                ...
                className={`w-full pl-3 pr-10 py-2 text-sm border rounded-md focus:outline-none focus:ring-1 ${startError ? 'border-red-400 focus:ring-red-400' : 'border-gray-300 focus:ring-[#7A0000]'} bg-white`}
              />
              <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5 pointer-events-none" />
            </div>
            {startError && <div className="mt-1 text-[11px] text-red-600">{startError}</div>}
          ...
          <div className="relative">
            ...
            <div className="relative">
              <input
                ...
                className={`w-full pl-3 pr-10 py-2 text-sm border rounded-md focus:outline-none focus:ring-1 ${endError ? 'border-red-400 focus:ring-red-400' : 'border-gray-300 focus:ring-[#7A0000]'} bg-white`}
              />
              <Calendar className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5 pointer-events-none" />
            </div>
            {endError && <div className="mt-1 text-[11px] text-red-600">{endError}</div>}
      ...
      {!loading && !error && filtered.length === 0 && <div className="text-gray-600 bg-gray-100 p-4 rounded">No departments found.</div>}
        ...
        <>
                  ...
                  {paginatedDepartments.map((d, i) => (
                    <tr 
                      ...
                      className={`${i % 2 ? "bg-white" : "bg-gray-50"} ${
                        selectedDepartment?.DeptId === d.DeptId ? "ring-2 ring-[#7A0000] ring-inset" : ""
                      }`}
                    >
                      ...
                      <td className="px-4 py-2 text-center">
                        ...
                        >
                          ...
                          {selectedDepartment?.DeptId === d.DeptId ? "Viewing" : "View"}
                        </button>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="flex justify-end items-center gap-3 mt-3">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
              className="px-3 py-1 border rounded bg-white text-gray-600 text-xs hover:bg-gray-100 disabled:opacity-40">Previous</button>
            <span className="text-xs text-gray-500">Page {page} of {Math.ceil(filtered.length / pageSize)}</span>
            <button onClick={() => setPage(p => Math.min(Math.ceil(filtered.length / pageSize), p + 1))}
              disabled={page >= Math.ceil(filtered.length / pageSize)}
              className="px-3 py-1 border rounded bg-white text-gray-600 text-xs hover:bg-gray-100 disabled:opacity-40">Next</button>
          </div>
        </>
      )}
          ...
          <div className="relative bg-white w-full max-w-lg rounded-xl shadow-2xl border border-gray-200 overflow-hidden">
            <div className="bg-white p-4 text-gray-800 border-b border-gray-200">
              <div className="flex justify-between items-center">
                ...
                <button onClick={closeDateRangeModal} className="text-gray-600 hover:text-gray-800 p-1 rounded-full hover:bg-gray-100">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            ...
            <div className="p-4 min-h-[420px] flex flex-col">
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="relative">
                  ...
                  {startDate && !isFuture(startDate) && daysAgo(startDate) > 0 && !startError && (
                ...
                <div className="relative">
                  ...
                  {endDate && !isFuture(endDate) && daysAgo(endDate) > 0 && !endError && (
                    <div className="mt-1 text-[11px] text-blue-700 bg-blue-50 border border-blue-200 rounded px-2 py-1">
              ...
              <div className="flex gap-2 justify-between items-center pt-4 border-t border-gray-200 mt-auto">
                <button onClick={closeDateRangeModal} className="px-4 py-2 text-gray-600 hover:text-gray-800 font-medium">Cancel</button>
                <button
                  onClick={() => handleViewCompletedProjects()}
                  disabled={!startDate || !endDate || !!startError || !!endError}
        ...
        <div className="fixed inset-0 bg-white flex items-start justify-end z-50 pt-24 pb-8 pl-64 pr-4">
          ...
          <div className="relative bg-white w-full max-w-6xl rounded-lg shadow-lg border border-gray-300 max-h-[85vh] flex flex-col mr-4">
            <div className="p-5 border-b bg-white sticky top-[1cm] z-20 mt-[1cm]">
              ...
              {completedError && <div className="text-red-600 text-xs mt-2">{completedError}</div>}
            </div>
            ...
            <div className="px-6 py-5 overflow-y-auto flex-grow">
              {completedLoading ? (
                <div className="flex items-center justify-center py-8">
                  ...
                  <span className="text-[#7A0000] text-sm">Loading completed projects...</span>
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-700 mb-2">No Completed Projects Found</h3>
                  <p className="text-gray-500 text-center max-w-md">
                    We couldn't find any completed projects for <strong>{selectedDepartment?.DeptName}</strong> in this period.
                  </p>
                </div>
              ) : (
                <div ref={printRef} className="w-full overflow-x-auto text-xs">
                  <div className="flex justify-between items-center p-2 bg-gray-50 border border-gray-200 rounded-t-lg">
                    ...
                    <span className="text-xs font-semibold text-gray-600">Total Records: {transformedData.length}</span>
                  </div>

                  <div className="border border-gray-200 rounded-b-lg">
                    <table className="w-full border-collapse">
                      <thead className="sticky top-[0px] text-gray-800" style={{backgroundColor: '#D3D3D3'}}>
                        <tr>
                          <th className="px-2 py-1 text-left border-b border-r">Account Code</th>
                          <th className="px-2 py-1 text-left border-b border-r">Project No</th>
                          <th className="px-2 py-1 text-left border-b border-r">Category Code</th>
                          <th className="px-2 py-1 text-left border-b border-r">Fund Id</th>
                          <th className="px-2 py-1 text-right border-b border-r">Stand Cost</th>
                          <th className="px-2 py-1 text-left border-b border-r">Description</th>
                          <th className="px-2 py-1 text-left border-b border-r">Dept_id</th>
                          <th className="px-2 py-1 text-left border-b border-r">PIV_No</th>
                          <th className="px-2 py-1 text-center border-b border-r">Close Date</th>
                          <th className="px-2 py-1 text-right border-b border-r">Labour</th>
                          <th className="px-2 py-1 text-right border-b border-r">Material</th>
                          <th className="px-2 py-1 text-right border-b border-r">Other</th>
                      ...
                      <tbody>
                        {transformedData.map((item, index) => (
                          <tr key={index} className="border-b hover:bg-gray-50">
                            <td className="px-2 py-1 border-r">{item.AccountCode}</td>
                            <td className="px-2 py-1 border-r font-mono">{item.ProjectNo?.trim()}</td>
                            <td className="px-2 py-1 border-r">{item.CategoryCode?.trim()}</td>
                            <td className="px-2 py-1 border-r font-mono">{item.FundId?.trim()}</td>
                            <td className="px-2 py-1 border-r text-right font-mono">
                              {item.StandCost?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
                            <td className="px-2 py-1 border-r max-w-xs" title={item.Description?.trim()}>
                              <div className="whitespace-normal break-words" style={{ fontSize: '10px', lineHeight: '1.2' }}>
                                {item.Description?.trim()}
                              </div>
                            </td>
                            <td className="px-2 py-1 border-r">{item.DeptId}</td>
                            <td className="px-2 py-1 border-r font-mono">{item.PIVNo || '-'}</td>
                            <td className="px-2 py-1 border-r text-center">
                              {item.CloseDate ? new Date(item.CloseDate).toLocaleDateString() : '-'}
                            </td>
                            <td className="px-2 py-1 border-r text-right font-mono">
                              {item.Labour?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
                            <td className="px-2 py-1 border-r text-right font-mono">
                              {item.Material?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
                            <td className="px-2 py-1 border-r text-right font-mono">
                              {item.Other?.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </td>
                      ...
                      <tfoot>
                        <tr className="bg-gray-100 border-t-2 border-gray-300">
                          ...
                          <td className="px-2 py-3 text-right font-bold text-gray-800 font-mono">
                            {transformedData.reduce((s, i) => s + (i.StandCost || 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                          <td colSpan={4}></td>
                          <td className="px-2 py-3 text-right font-bold text-gray-800 font-mono">
                            {transformedData.reduce((s, i) => s + (i.Labour || 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                          <td className="px-2 py-3 text-right font-bold text-gray-800 font-mono">
                            {transformedData.reduce((s, i) => s + (i.Material || 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                          <td className="px-2 py-3 text-right font-bold text-gray-800 font-mono">
                            {transformedData.reduce((s, i) => s + (i.Other || 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                          <td className="px-2 py-3 text-right font-bold text-[#7A0000] text-sm font-mono">
                            {transformedData.reduce((s, i) => s + (i.Total || 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                        </tr>
                      </tfoot>
                    </table>
