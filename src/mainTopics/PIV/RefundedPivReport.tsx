// Refunded PIV Details (Refunded within selected period, specific cost center)
...
const RefundedPivReport: React.FC = () => {
  ...
  const handleViewReport = async (dept: {
    ...
    try {
      ...
      items.length === 0
        ? toast.warn("No refunded PIV records found for selected period and cost centre")
  ...
  const handleDownloadCSV = () => {
    ...
    reportData.forEach((item) => {
      ...
      if (item.RefundableAmount != null) grandTotalRefundable += item.RefundableAmount;
    });
            ...
            <table className="w-full text-xs min-w-max border-collapse">
              <thead className={`${maroonGrad} text-white`}>
                <tr>
                  ...
                  <th className="px-3 py-2 text-right">PIV Paid Amount</th>
                  <th className="px-3 py-2 text-right">Refundable Amount</th>
                </tr>
              </thead>
              <tbody>
                {reportData.map((item, idx) => (
                  ...
                  >
                    <td className="px-3 py-1 border border-gray-200">{item.PivNo || "-"}</td>
                    <td className="px-3 py-1 border border-gray-200">{formatDate(item.PaidDate)}</td>
                    <td className="px-3 py-1 border border-gray-200">{formatDate(item.PivDate)}</td>
                    <td className="px-3 py-1 border border-gray-200">{item.Name || "-"}</td>
                    <td className="px-3 py-1 border border-gray-200">{item.Address || "-"}</td>
                    <td className="px-3 py-1 border border-gray-200">{item.AccountCode || "-"}</td>
                    <td className="px-3 py-1 border border-gray-200">{formatDate(item.RefundDate)}</td>
                    <td className="px-3 py-1 text-right border border-gray-200">{formatNumber(item.GrandTotal)}</td>
                    <td className="px-3 py-1 text-right border border-gray-200">{formatNumber(item.RefundableAmount)}</td>
                  </tr>
                ))}
