...
interface PIVBanksItem {
	Issued_cost_center: string;
	Account_code: string;
	Amount: number;
	Company_name: string;
	Issued_cost_center_name: string;
}
...
const PivByBanks: React.FC = () => {
	useUser();

	const [fromDate, setFromDate] = useState("");
	const [toDate, setToDate] = useState("");
	const [reportData, setReportData] = useState<PIVBanksItem[]>([]);
	...
	// Hierarchical grouping: Company → Issued Cost Center
	const groupedData = useMemo(() => {
		...
		reportData.forEach((item) => {
			...
			const ccKey = `${item.Issued_cost_center}|||${item.Issued_cost_center_name}`;
			...
			if (!cc) {
				cc = {
					id: item.Issued_cost_center,
					name: item.Issued_cost_center_name,
					amounts: {},
				};
	...
	const handleViewReport = async () => {
		...
		try {
			const url = `/misapi/api/pivbybanks/get?fromDate=${fromDate.replace(
				/-/g,
				""
			...
			const items: PIVBanksItem[] = Array.isArray(data)
				? data
				: data.data || [];
			setReportData(items);
			items.length === 0
	...
	const handleDownloadCSV = () => {
		...
		link.href = url;
		link.download = `PIV_Collections_by_Banks_${fromDate}_to_${toDate}.csv`;
		link.click();
...
<head>
<meta charset="utf-8">
<title>PIV Collections by Banks</title>
<style>${tableStyle}
...
<body>
<h3>PIV Collections by Banks</h3>
<div class="subtitle">
  <strong>Period :</strong> ${fromDate} to ${toDate}<br/>
...
<table>${colGroupHTML}${headerHTML}<tbody>${bodyHTML}</tbody></table>
	...
	return (
		<div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow border border-gray-200 text-sm">
			...
			{showReport && (
				<ReportViewer
					title="07. PIV Collections by Banks"
					subtitlebold="Period:"
					...
					<table className="w-full text-xs min-w-max">
						...
						<tbody>
							{groupedData.map(([company, group]) => {
								const {costCenters, companyTotals, companyRowTotal} =
									group;
								return (
							...
							<tr className="bg-blue-200 font-bold text-sm">
								<td
									colSpan={2}
									className="px-3 py-2 border border-white text-left"
								>
									GRAND TOTAL
								</td>
								{accountCodes.map((code) => (
