// File: ProvincialConsolidatedOutputVAT.tsx
...
const ProvincialConsolidatedOutputVAT: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<ProvincialVATItem[]>([]);
	...
	const handleViewReport = async (company: Company) => {
		...
		try {
			...
			const url = `/misapi/api/provincial-consolidated-vat/report?fromDate=${apiFrom}&toDate=${apiTo}&compId=${company.compId.trim()}`;
			const res = await fetch(url);
			if (!res.ok) throw new Error(`HTTP ${res.status}`);

			const data = await res.json();
			const items: ProvincialVATItem[] = Array.isArray(data) ? data : [];
			...
			items.length === 0
				...
				: toast.success(
						"Consolidated Output VAT Schedule loaded",
					);
		} catch (err: any) {
	...
	const handleDownloadCSV = () => {
		...
		link.href = url;
		link.download = `Provincial_Consolidated_Output_VAT_${selectedCompany.compId}_${fromDate}_to_${toDate}.csv`;
		link.click();
...
<div class="subtitles">
  <div class="subtitle-left">
    <div><strong>Company / Branch:</strong> ${escapeHtml(selectedCompany.compId)} / ${escapeHtml(selectedCompany.CompName)}</div>
...
export default ProvincialConsolidatedOutputVAT;
