// File: ProvinceWisePIVStampDutyReport.tsx
...
interface PIVStampDutyItem {
	Paid_Date: string | null; // ISO string from backend
	Pay_type: string;
	Count: number;
	Amount: number;
	Stamp_Duty: number;
...
const ProvinceWisePIVStampDuty: React.FC = () => {
	const {user} = useUser();
	...
	const [reportData, setReportData] = useState<PIVStampDutyItem[]>([]);
	...
	const handleViewReport = async (company: Company) => {
		...
		try {
			const url = `/misapi/api/provincepivstampduty/get?compID=${company.compId.trim()}&fromDate=${fromDate.replace(
				/-/g,
				""
			...
			const items: PIVStampDutyItem[] = Array.isArray(data)
				? data
				: data.data || [];
			setReportData(items);
			items.length === 0
	...
	const handleDownloadCSV = () => {
		...
		// Add Total Stamp duty for the period row
		csvRows.push("");
		csvRows.push(
		...
		link.href = url;
		link.download = `PIV_Stamp_Duty_${selectedCompany.compId}_${fromDate}_to_${toDate}.csv`;
		link.click();
...
<head>
<meta charset="utf-8">
<title>Stamp Duty for PIV Transactions</title>
<style>${tableStyle}
...
<div class="subtitles">
  ...
  <div><strong>Company / Branch:</strong> ${escapeHtml(
...
export default ProvinceWisePIVStampDuty;
