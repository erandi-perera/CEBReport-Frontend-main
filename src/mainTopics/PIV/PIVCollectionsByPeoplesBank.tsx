...
const PivByPeopleBank: React.FC = () => {
	useUser();
	...
	const handleViewReport = async () => {
		...
		try {
			const url = `/misapi/api/pivbypeoplebank/get?fromDate=${fromDate.replace(
				/-/g,
				""
	...
	const handleDownloadCSV = () => {
		...
		const csvRows: string[] = [
			"PIV Collections by Peoples Bank",
		...
		link.href = url;
		link.download = `PIV_Collections_by_PeoplesBank_${fromDate}_to_${toDate}.csv`;
		link.click();
...
<head>
<meta charset="utf-8">
<title>PIV Collections by People's Banks</title>
<style>${tableStyle}
...
<body>
<h3>PIV Collections by People's Banks</h3>
<div class="subtitles">
	...
	return (
		<div className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow border border-gray-200 text-sm">
			<h2 className={`text-xl font-bold mb-4 ${maroon}`}>
				PIV Collections by People's Banks
			</h2>
...
export default PivByPeopleBank;
